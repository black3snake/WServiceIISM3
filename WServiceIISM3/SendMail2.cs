...
{
    internal class SendMail2
    {
        internal ExchangeService Service { get; private set; }
        internal delegate void AccountHandler(string message);
        internal event AccountHandler Notify;
        internal SendMail2(ExchangeService _service) => Service = _service;

        internal void SendM(Data data, string ServerName)
        {
            ...
            message.Send();
            Notify?.Invoke("An email with the subject '" + message.Subject + "' has been sent to '" + message.ToRecipients[0] + "' and saved in the SendItems folder.");
